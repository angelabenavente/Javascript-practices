...
const result = adalabers.filter(adalaber => adalaber.profession === 'diseñadora')
...
/*function profe(adalabers){
  for(let i = 0; i < adalabers.length; i++){
    let adalaber = [adalabers[i]];
    for(let i = 0; i < adalaber.length; i++) {
      if (ada.professions[i]==='diseñadora') {
        designersNumber = designersNumber +1;
      }
    }
  ...
  function getNumber(profession, adalabers) {
    let designers = 0;

    for(let i = 0; i < adalabers.length; i++) {
      if ('profession' in adalabers[i] && adalabers[i].profession === profession) {
        designers++;
      }
      return designers;
    }
...
console.log(getNumber('diseñadora', adalabers));